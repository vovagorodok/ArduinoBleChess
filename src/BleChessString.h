...
class BleChessStringViewImpl {
 public:
...
#if __cpp_lib_string_view
    constexpr operator std::string_view() const { return std::string_view(_data, _size); }
#endif
    inline operator std::string() const { return std::string(_data, _size); }
#ifdef ARDUINO
    inline operator String() const { return String(_data, _size); }
#endif
    ...
    constexpr BleChessStringViewImpl substr(size_t pos) const {
        return (pos > _size) ? BleChessStringViewImpl() : BleChessStringViewImpl(_data + pos, _size - pos);
    }
    constexpr BleChessStringViewImpl substr(size_t pos, size_t count) const {
        return (pos > _size) ? BleChessStringViewImpl() :
    ...
    BLE_CHESS_CONSTEXPR bool operator==(const char* str) const { return operator==(BleChessStringViewImpl(str)); }
    inline bool operator==(const std::string& str) const { return operator==(BleChessStringViewImpl(str)); }
#ifdef ARDUINO
    inline bool operator==(const String& str) const { return operator==(BleChessStringViewImpl(str)); }
#endif
 ...
 private:
    static BLE_CHESS_CONSTEXPR size_t _strlen(const char* str) { return str ? std::char_traits<char>::length(str) : 0; }
