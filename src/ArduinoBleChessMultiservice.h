...
#if defined(BLE_CHESS_BLE_LIB_FAKE)
...
inline BleChessServerFake* initBle(const std::string& deviceName) {
    return nullptr;
}
inline bool advertiseBle(BleChessServerFake* server, const std::string& deviceName, const std::string& primaryUUID,
...
#elif defined(BLE_CHESS_BLE_LIB_ARDUINO_BLE)
inline bool initBle(const char* deviceName) {
    if (!BLE.begin()) {
        return false;
    }
...
#if defined(BLE_CHESS_BLE_LIB_FAKE)
using BleChessServer = BleChessServerFake;
#elif !defined(BLE_CHESS_BLE_LIB_ARDUINO_BLE)
using BleChessServer = BLEServer;
#endif
