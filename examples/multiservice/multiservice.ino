...
class LedCharacteristicCallbacks : public BLECharacteristicCallbacks {
 public:
    void onWrite(BLECharacteristic* characteristic) override { digitalWrite(LED_BUILTIN, *characteristic->getData()); }
};
LedCharacteristicCallbacks ledCallbacks;
#endif
...
void setup() {
    ...
    auto ledCharacteristic = ledService->createCharacteristic(
        LED_CHARACTERISTIC_UUID, BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_WRITE);
    uint8_t value = 0;
