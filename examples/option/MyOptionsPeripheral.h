...
class MyOptionsPeripheral : public MyPeripheral {
 public:
    ...
    void handleCentralOptionsBegin() override {
        ...
        sendPeripheralOption(join(optionEnum.name, BleChessOptionType::Enum,
                                  (optionEnum.defaultValue == Value1 ? value1 : value2), value1, value2));
        sendPeripheralOption(join(optionStr.name, BleChessOptionType::Str, optionStr.defaultValue));
        sendPeripheralOption(join(optionInt.name, BleChessOptionType::Int,
                                  std::to_string(optionInt.defaultValue).c_str(), std::to_string(0).c_str(),
                                  std::to_string(100).c_str()));
        sendPeripheralOption(join(optionFloat.name, BleChessOptionType::Float,
                                  std::to_string(optionFloat.defaultValue).c_str(), std::to_string(0.0).c_str(),
                                  std::to_string(20.0).c_str()));

        if (optionBool.value != optionBool.defaultValue) {
            sendPeripheralSetOption(join(
                optionBool.name, (optionBool.value ? BleChessBoolOptionValue::True : BleChessBoolOptionValue::False)));
        }
